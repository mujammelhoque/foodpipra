...
<div class="container">
  ...
  <tbody>
    ...
    <tr>
      <th class="bg-secondary  text-light">Discription</th>
      <td class="bg-info text-light">{{ $product->description}}</td>
    </tr>
  ...
  <form action="{{ route('seller.product.destroy',$product->id) }}" method="POST">
    ...
    <button Onclick="return ConfirmDelete()" type="submit" class="btn btn-outline-danger">Delete</button>
     &nbsp;<a class="btn btn-info" href="{{ route('seller.product.index') }}">Back</a>
  </form>
</div>
@endsection
